...
async function getContent(){
  const html = await fetch('http://localhost:8888/ProyectoMVC_Parte3/expenses/create').then(res => res.text());
  return html;
}
...
google.charts.load('current', {'packages':['bar']});
      ...
      async function drawChart() {
        const http = await fetch('http://localhost:8888/ProyectoMVC_Parte3/expenses/getExpensesJSON')
        .then(json => json.json())
        .then(res => res);
